...
// const size4326 = projectionExtent4326 ? getWidth(projectionExtent4326) / 256 : 0;
...
// const size3857 = projectionExtent3857 ? getWidth(projectionExtent3857) / 256 : 0;
...
function Newmap() {
  ...
  useEffect(() => {
    ...
    // Add the OL‑ext LayerSwitcher control
    // const layerSwitcher = new LayerSwitcher({
    //   startActive: true,
    //   groupSelectStyle: "children",
    // } as any);
    ...
    // // Listen for layer visibility changes to update activeLayerName
    // dynamicGroups.forEach((group) => {
  ...
  const computedLayerGroups = wmtsLayers.reduce((groups: Record<string, any[]>, layer: any) => {
    const groupTitle = layer.group?.groupTitle || "Ungrouped";
    if (!groups[groupTitle]) {
      groups[groupTitle] = [];
    }
    ...
    <Collapse in={overlaysOpen} timeout="auto" unmountOnExit>
      {Object.entries(computedLayerGroups).map(([groupTitle, groupLayers]) => (
        <div key={groupTitle}>
          ...
          >
            <ListItemIcon>
              <FolderIcon style={{ color: expandedGroups[groupTitle] ? green[500] : "inherit" }} />
            </ListItemIcon>
            <ListItemText
              primary={groupTitle}
            ...
            />
            {expandedGroups[groupTitle] ? <ChevronLeftIcon sx={{ color: green[500] }} /> : <ChevronRightIcon />}
          </ListItemButton>
          <Collapse in={expandedGroups[groupTitle]} timeout="auto" unmountOnExit>
            <List component="div" disablePadding>
              {groupLayers.map((layer) => {
                ...
                return (
                  <ListItemButton key={layer.title} sx={{ pl: 8 }} onClick={() => handleLayerToggle(olLayer)}>
                    <Checkbox edge="start" checked={olLayer?.getVisible() || false} tabIndex={-1} disableRipple />
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100vh" }}>
      <CssBaseline />
      <AppBar position="fixed" open={open} sx={{ bgcolor: "white", margin: 0, padding: 0 }}>
        <Toolbar>
          <IconButton
            color="inherit"
          ...
          <ListItem disablePadding>
            ...
            >
              <ListItemButton
                ...
                sx={{
                  "&:hover": { backgroundColor: alpha(theme.palette.primary.main, 0.1) },
                  backgroundColor: downloadPopupOpen ? alpha(green[500], 0.1) : "transparent",
                }}
              >
                <ListItemIcon>
                  <InboxIcon sx={{ color: downloadPopupOpen ? green[500] : "inherit" }} />
                </ListItemIcon>
                <ListItemText
                  primary="Download"
                  primaryTypographyProps={{
                ...
                />
                {downloadPopupOpen ? <ChevronLeftIcon sx={{ color: green[500] }} /> : <ChevronRightIcon />}
              </ListItemButton>
              <Collapse
